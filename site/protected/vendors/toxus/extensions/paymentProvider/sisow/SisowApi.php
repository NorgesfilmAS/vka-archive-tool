...
class SisowApi
{
	...
	public function Overboeking($trxid = false)
	{
		$this->trxId = $this->issuerUrl = "";
		if (!$this->merchantId)
			return -1;
		...
		$pars["callbackurl"] = $this->callbackUrl;
		$pars["notifyurl"] = $this->notifyUrl;
		$pars['including'] = $this->email;
		$pars['billing_mail'] = $this->email;
		$pars['days'] = $this->days;
		$pars["sha1"] = sha1($this->purchaseId . $this->entranceCode . round($this->amount * 100) . $this->merchantId . $this->merchantKey);
		if ($keyvalue) {
			foreach ($keyvalue as $k => $v) {
		...
		$this->documentid = urldecode($this->parse("documentid"));
		$this->documenturl = urldecode($this->parse("documenturl"));
