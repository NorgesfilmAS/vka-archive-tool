<?php
/**
 * Paypal API PHP wrapper class
 * 
 * See included examples in /examples/ for usages instructions and examples.
 * 
 * This class can only be used with a license purchased from Binpress
 * @link http://www.binpress.com/app/php-paypal-api-class/20
 * @copyright Lionite
 * 
 */
class Lionite_Paypal {
	 ...
	 * - Used to confirm identity of endpoint (Paypal)
	 *
	 * Latest can be obtained from Mozilla at 
	...
	/**
	 * Paypal Authentication Certificate file location (alternative authentication method)
	 * - Either relative or absolute path
	 * - If using a certificate, remove the 'signature' parameter from the $_settings array 
	 * 
	 * @var string
	 */
	...
	/**
	 * Item key values used in Paypal transactions
	 * - Normalizes item keys used by paypal to lowercase / descriptive keys
	 *
	 * @var array
	 */
	...
	/**
	 * Payment options key values used in Paypal transactions
	 * - Normalizes payment options keys used by Paypal to lowercase / descriptive keys
	 * 
	 * @var array
	 */
    ...
    /**
     * Express checkout specific payment options
	 ...
	 * 
	 * Required parameters are covered under the createRecurringProfile() method documentation
	 * For more details on optional parameters see https://cms.paypal.com/us/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_api_nvp_r_CreateRecurringPayments
	 * 
	 */
	 ...
	 *
	 * Returned from getRecurringProfile()
	 ...
	 * - Additional parameters for direct payment. Unless marked otherwise, parameters are required
	 * 
	 * @var array
	 */
	 ...
	 * @see getExpressCheckoutToken() for more details
	 *  
	 * @param array $options
	 * @param array $items
	 * @return string / array Either a checkout URL or an array of errors
	 */
	...
	/**
	 * Prepare checkout token
	 * - The $options array parameters are specified in $_paymentOptions. 'cancel' and 'return' are required
	 * - The $items parameter can be passed either one item directly or an array of items
	 * - Each item must have a 'cost' key. Additional parameters include description, item amount, shipping and more. 
	 * - If no items are provided through $items, a 'cost' parameter must be provided through the $options array
	 * 
	 * @see $_paymentOptions for more details on the various payment options
	 * @see $_itemParams for more details on the various item keys
	 * 
	 * @param array $options
	 * @param array $items
	 * @return string / boolean Express Checkout token / Request failure
	 */
    ...
    /**
     * Build Express Checkout NVP array
     * 
     * @see getExpressCheckoutToken() for details on the parameters
     * @param array $options
     * @param array $items
     * @return array 
     */
