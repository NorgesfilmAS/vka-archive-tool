...
//	die('Access only from the localhost: '.$_SERVER['REMOTE_ADDR']);
...
if (isset( $_REQUEST['id'])) {
	...
	if (isset($_REQUEST['altId'])) {
		/**
		 * we are generating the images connected to the alternative 
		 */
		$altId = $_REQUEST['altId'];
		if (!is_numeric($altId)) die('The altId must be nummeric');
		
		$model = sql_query('SELECT file_extension FROM resource_alt_files WHERE ref='.$altId);
		if ($model) {
			$extension = $model[0]['file_extension'];
		} else {
			die('Resource not found.');
		}
		
		create_previews($id, false, $extension, false, false, $altId);		
		echo "Alternate file updated for resource $id, alternate id: $altId";
	} else {
		// extract metadata
		...
		echo "File added / updated for resource $id";
	}	
